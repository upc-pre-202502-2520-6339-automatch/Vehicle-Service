...
import com.vehicle.domain.model.commands.*;
...
public class VehicleCommandServiceImpl {
    ...
    public void handle(DeleteVehicleCommand command) {
        ...
        // Reglas de dominio sugeridas
        if (vehicle.getStatus() == VehicleStatus.SOLD) {
            throw new IllegalArgumentException("Sold vehicles cannot be deleted");
        }
        if (vehicle.getStatus() == VehicleStatus.WITHDRAWN) {
            // idempotente: ya retirado, no haces nada
            return;
        }

        vehicle.retire();                  // cambia a WITHDRAWN (ya lo tienes en el agregado)
        vehicleRepository.save(vehicle);

        // Reutiliza tu evento de actualizado (o crea uno VehicleWithdrawnEvent si quieres)
        eventProducer.publishUpdated(new VehicleUpdatedEvent(vehicle.getId()));
    }
